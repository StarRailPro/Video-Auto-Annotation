...
class MCPResponseParser:
    ...
    def parse(response: Dict[str, Any]) -> Dict[str, Any]:
        if not isinstance(response, dict):
            ...
            return {"description": str(response), "tags": [], "confidence_scores": None}
        ...
        if response.get("isError"):
            ...
            return {"description": f"MCP Error: {error_text}", "tags": [], "confidence_scores": None}

        if "error" in response:
            error = response["error"]
            error_msg = error.get("message", "Unknown error") if isinstance(error, dict) else str(error)
            logger.error(f"MCP error response: {error_msg}")
            return {"description": f"MCP Error: {error_msg}", "tags": [], "confidence_scores": None}
        ...
        if "result" in response and "content" not in response:
            ...
            if isinstance(result, dict) and "content" in result:
                ...
                return {"description": str(result) if result else "Empty result", "tags": [], "confidence_scores": None}

        if "content" not in response:
            logger.warning(f"No 'content' in response. Keys: {list(response.keys())}")
            return {"description": "No content in MCP response", "tags": [], "confidence_scores": None}

        content = response["content"]

        if not isinstance(content, list):
            logger.warning(f"Content is not a list: {type(content)}")
            return {"description": str(content), "tags": [], "confidence_scores": None}
        ...
        if not text_contents:
            ...
            return {"description": "No text content in MCP response", "tags": [], "confidence_scores": None}
        ...
        try:
            ...
            logger.info("Response is not JSON, using as plain text")
            return {"description": combined_text, "tags": [], "confidence_scores": None}
...
def analyze_with_mcp(
    ...
    Args:
        ...
        predefined_tags: Optional list of tag values to use in the prompt.
                        If None, falls back to TagEnum.get_all_values().
        
    Returns:
        Dictionary with analysis results
    """
    tag_values = predefined_tags if predefined_tags is not None else TagEnum.get_all_values()
    ...
    prompt = PROMPT_TEMPLATE.format(
        ...
        predefined_tags=", ".join(tag_values)
    )

    image_paths = video_data.get("keyframe_paths", [])

    if not image_paths:
        logger.warning(f"No keyframes for {video_data['file_path']}. Returning default result.")
        default_tag = tag_values[0] if tag_values else "normal_operation"
        return {
            "description": f"Video processed but no keyframes extracted for {video_data['file_path']}.",
            "tags": [default_tag],
...
def analyze_video_with_ai(
    ...
    Args:
        ...
        predefined_tags: Optional list of tag values for validation and prompt generation.
                        If None, falls back to TagEnum.get_all_values().

    Returns:
        A dictionary containing the AI analysis result:
        - "description": str (max 50 words)
        - "tags": List[str] (from predefined tags)
    ...
    try:
        api_response_payload = analyze_with_mcp(video_data, client, predefined_tags=tag_values)
